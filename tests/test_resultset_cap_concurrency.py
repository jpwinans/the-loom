...
instead of the true default (10000) — permanently stranding the server.
...
def test_concurrent_runs_leave_resultset_size_at_default(db: FalkorDB) -> None:
    ...
    assert int(before) == 10000, (
        f"RESULTSET_SIZE was already {before!r} before this test ran (expected the "
        "true default 10000) — some other run left the server stranded already; "
        "restore it (GRAPH.CONFIG SET RESULTSET_SIZE 10000) before re-running this "
        "test, which otherwise cannot prove anything from a dirty baseline."
    )
    ...
    assert int(after) == 10000, (
        f"RESULTSET_SIZE ended at {after!r} instead of the true default 10000 — "
        "the fixture stranded the server under concurrent runs.\n\n"
        f"run 1 output:\n{outputs[0]}\n\nrun 2 output:\n{outputs[1]}"
    )
